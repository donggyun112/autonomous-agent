...
describe("session store", () => {
    it("append + load round-trips messages", async () => {
        const { appendMessage, clearSession, loadSession, } = await import("../core/session-store.js");
        await clearSession();
        const msg1 = { role: "user", content: "Hello" };
        const msg2 = { role: "assistant", content: [{ type: "text", text: "Hi back" }] };
        await appendMessage(msg1);
        await appendMessage(msg2);
        const loaded = await loadSession();
        expect(loaded.length).toBe(2);
        expect(loaded[0].content).toBe("Hello");
        expect(loaded[1].content[0].text).toBe("Hi back");
    });
    ...
    it("replaceSession overwrites", async () => {
        const { appendMessage, replaceSession, loadSession, } = await import("../core/session-store.js");
