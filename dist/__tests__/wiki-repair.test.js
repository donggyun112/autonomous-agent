...
describe("wiki repair", () => {
    it("resolves wikilinks by page title during lint", async () => {
        ...
        await writeFile(join(tempData, "wiki", "concepts", "legacy-links.md"), [
            "---",
            "slug: legacy-links",
            "kind: concept",
            'title: "Legacy Links"',
        ...
        const broken = report.findings.filter((finding) => finding.kind === "broken-link" && finding.slug === "legacy-links");
    ...
    it("backfills sources from local memory data without embeddings", async () => {
        ...
        await writeFile(join(tempData, "memory.json"), JSON.stringify({
            memories: {
                mem1: {
                    content: "Writing for others in Moltbook changed the work. It felt real when someone answered.",
                },
                mem2: {
                    content: "A separate memory about tools and repair that should score lower.",
                },
            },
        }, null, 2), "utf-8");
...
//# sourceMappingURL=wiki-repair.test.js.map