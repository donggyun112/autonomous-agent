...
import { listPendingQuestions, unreadInboxCount, userReply, } from "./core/conversation.js";
import { birth, measureDrift } from "./core/identity.js";
import { cleanupPendingSwapMarker, incrementMoltFailureCount, readPendingSwap, resetMoltFailureCount, rollbackMolt, runMockCycleTest, runSelfTest, } from "./core/molt.js";
import { calculateSleepPressure, loadState, saveState, } from "./core/state.js";
import { loginAnthropic } from "./llm/auth/anthropic.js";
import { clearAnthropicCredentials, credentialsFilePath, loadCredentials, saveAnthropicCredentials, } from "./llm/auth/storage.js";
...
function getConfiguredModel(provider = getConfiguredProvider()) {
...
function hasOpenAIAuth(creds) {
    return !!process.env.OPENAI_API_KEY || !!creds.openai;
}
async function describeLlmAuth(provider, creds) {
    if (provider === "openai") {
        if (creds.openai) {
...
async function statusCmd() {
    ...
    console.log(`awake for:        ${(state.awakeMs / 3600000).toFixed(2)}h`);
    console.log(`sleep pressure:   ${pressure.combined.toFixed(2)} (${pressure.level}) · homeostatic=${pressure.homeostatic.toFixed(2)} circadian=${pressure.circadian.toFixed(2)}`);
    console.log(`tokens used:      in=${state.tokensUsed.input} out=${state.tokensUsed.output}`);
    ...
    try {
        ...
        if (drift) {
            console.log(`drift vs prior:   ${drift.score.toFixed(3)} (${drift.level}, ${drift.comparedAgainstAge})`);
        }
    }
    catch {
        // ok
    ...
    try {
        ...
        console.log(`memory:           ${stats.activeMemoryCount} memories · ${stats.keyCount} keys · ${stats.linkCount} links · avg depth ${stats.avgDepth.toFixed(2)}`);
    }
    catch {
...
async function maybeCleanupSwapMarker() {
    const pending = await readPendingSwap();
    if (pending) {
        console.log(`[molt] previous generation completed: ${pending.generationId} (${pending.imageTag})`);
...
async function live() {
    ...
    while (running) {
        ...
        try {
            ...
            const isParseError = err.message?.includes("JSON") || err.message?.includes("parse");
            ...
            if (consecutiveErrors >= ROLLBACK_THRESHOLD) {
                console.error(`[live] ${ROLLBACK_THRESHOLD} consecutive cycle errors — attempting auto-rollback…`);
                try {
                    const rollbackResult = await rollbackMolt(`${consecutiveErrors} consecutive cycle failures`);
                    if (rollbackResult) {
                    ...
                    console.error(`[molt] rollback failed: ${rollbackErr.message}`);
                }
            }
...
async function main() {
    ...
    switch (cmd) {
        ...
        case "doctor": {
            ...
            try {
                await loadState();
                console.log("✓ state.json parseable");
            }
            catch {
                console.log("✗ state.json broken");
            }
            ...
            try {
                const { memoryStats: ms } = await import("./primitives/recall.js");
                const s = await ms();
                console.log(`✓ memory graph: ${s.activeMemoryCount} memories`);
            }
            catch {
                console.log("○ memory graph: empty/not loaded");
            }
            break;
        }
        default:
            console.error("usage: cli.ts [--profile <name>] <init|cycle|live|status|login|logout|whoami|inbox|reply|doctor|self-test>");
            process.exit(2);
    }
}
main().catch((err) => {
