...
import { resolveProviderConfig, think } from "../llm/client.js";
        ...
        if (typeof m.content === "string") {
            ...
            for (const block of m.content) {
                if ("text" in block) {
                    ...
                    parts.push(`${m.role}/tool_use: ${block.name}(${JSON.stringify(block.input)})`);
                }
            }
        }
    }
...
function extractToolActions(messages) {
    const actions = [];
    for (const m of messages) {
...
function findSplit(messages, keepRecentTokens) {
    let kept = 0;
...
function prePruneToolOutputs(messages, splitAt) {
    if (splitAt <= 0)
        return messages;
    const toolIdMap = buildToolUseIdMap(messages.slice(0, splitAt));
    return messages.map((m, i) => {
        if (i >= splitAt)
            return m; // retained verbatim — don't touch
        if (m.role !== "user")
            return m;
        if (typeof m.content === "string")
            return m;
        ...
        return {
            ...
            content: m.content.map((b) => {
                if ("type" in b && b.type === "tool_result") {
                    const toolUseId = "tool_use_id" in b ? b.tool_use_id : "";
...
function microcompact(messages, keepRecentCount) {
        ...
        return {
            ...
            content: m.content.map((b) => {
                if ("type" in b && b.type === "tool_result") {
                    ...
                    if (oldLen <= PRUNED_PLACEHOLDER.length)
                        return b; // already pruned
...
function truncateOldest(messages, targetTokens) {
...
export async function compactIfNeeded(messages, systemPromptForContext, options) {
    ...
    const estimatedRequestTokens = totalTokens + systemPromptTokens + reservedCompletionTokens + toolDefsTokens;
    const { triggerTokens, keepRecentTokens } = getContextBudget();
    if (estimatedRequestTokens < triggerTokens)
        return null;
    ...
    promptParts.push("", "--- tool actions ---", toolActionsSummary, "--- end tool actions ---", "", "--- older messages ---", messagesAsText(older), "--- end ---");
    let summaryText;
    try {
    ...
    let newMessages = [synthetic, ...recent];
