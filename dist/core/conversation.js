...
function nowTsFilename() {
    return new Date().toISOString().replace(/[:.]/g, "-");
}
export async function askUser(args) {
    if (!args.question?.trim())
        throw new Error("askUser: question is required");
    if (!args.reason?.trim())
        throw new Error("askUser: reason is required (why are you asking?)");
    const id = newId();
...
export async function writeLetter(args) {
    if (!args.text?.trim())
        throw new Error("writeLetter: text is required");
    const id = newId();
    const ts = nowTsFilename();
        ...
        if (!includeAll) {
            ...
            if (Number.isFinite(ms)) {
                ...
                try {
                    ...
                    if (fileStat.mtimeMs <= state.lastInboxReadAt)
                        continue;
                }
                catch {
...
export async function userReply(args) {
    if (!args.text?.trim())
        throw new Error("userReply: text is required");
    const id = newId();
    const ts = nowTsFilename();
