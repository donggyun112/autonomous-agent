...
import { calculateSleepPressure, FORCE_THRESHOLD, MIN_HOMEOSTATIC_FOR_SLEEP, MIN_SLEEP_THRESHOLD, loadState, saveState, tickAwake, transition, } from "./state.js";
import { readToday, readYesterday } from "../memory/journal.js";
import { reconstitute, measureDrift } from "./identity.js";
import { toolsForMode, toolDefs, dispatchTool, resetActivatedTools, extendedToolNames } from "./tools.js";
...
import { extensionsSummary, loadExtensionTools, } from "./extensions.js";
import { appendMessage, clearSession, loadSession, replaceSession, initSessionMeta, } from "./session-store.js";
import { runSleepConsolidation } from "./sleep.js";
...
export async function runCycle(options) {
    ...
    const pressureNote = pressure.combined >= 0.25
        ? [
            "---",
            "## body",
            "",
    ...
    if (state.mode === "WAKE" && (state.wakeIntention || state.wakeContext)) {
        ...
        parts.push("", "This is what you-before-sleep wanted you-after-sleep to know. Pick it up if it still matters.");
    ...
    const coreTools = await toolsForMode(state.mode);
    ...
    let tools = [...coreTools, ...extensionTools];
    ...
    let messages = await loadSession();
    if (messages.length === 0) {
        ...
        openingParts.push("\nBegin.");
        const opening = { role: "user", content: openingParts.join("\n") };
        messages = [opening];
    ...
    const maybeCompactConversation = async () => {
        try {
            ...
            observer?.onToolStart?.("(auto-compact)", { before: compacted.before, after: compacted.after, summarized: compacted.summarizedCount });
            observer?.onCompaction?.({ before: compacted.before, after: compacted.after });
        }
        catch (err) {
            observer?.onToolEnd?.("(auto-compact)", `failed: ${err.message}`);
        }
    };
    for (let turn = 0; turn < maxTurns; turn++) {
        ...
        for (const call of response.toolCalls) {
            ...
            try {
                ...
                out = `(tool error: ${dispatchErr.message})`;
                rawOut = out;
            }
            ...
            if (out.startsWith("(tool error:")) {
                try {
                    await enqueueFailed({ tool: call.name, input: call.input, error: out, ts: new Date().toISOString() });
                }
                catch { }
            }
        ...
        if (transitionRequested) {
            ...
            if (wasInSleep && transitionRequested.to === "WAKE") {
                try {
                    ...
                    observer?.onToolEnd?.("(sleep-auto)", err.message);
                }
    ...
    endSpan(cycleSpan);
    try {
        await saveTrace(state.sleepCount, state.cycle);
    }
    catch { }
