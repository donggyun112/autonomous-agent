...
function looksLikeTool(x) {
    if (!x || typeof x !== "object")
        return false;
    const t = x;
    if (!t.def || typeof t.def !== "object")
        return false;
    const def = t.def;
        ...
        try {
            ...
            for (const c of candidates) {
                if (looksLikeTool(c)) {
                    ...
                    invalid.push(typeof c.def?.name === "string"
                        ? c.def.name
                        : "(unnamed)");
                }
            }
            loaded.push({
                ...
                file: full,
                tools: valid,
                error: invalid.length > 0 && valid.length === 0
...
export function extensionsSummary(loaded) {
    if (loaded.length === 0) {
    ...
    if (errored.length > 0) {
        parts.push(`${errored.length} failed to load: ${errored.map((e) => `${e.name} (${e.error})`).join("; ")}`);
    }
