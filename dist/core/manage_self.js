...
import { copyFile, mkdir, readFile, readdir, stat, writeFile, appendFile, } from "fs/promises";
...
const ALLOWED_SCOPES = Object.keys(SCOPES);
function resolveTarget(scope, name) {
...
export async function manageSelf(action) {
    ...
    if (action.kind === "list") {
        ...
        try {
            ...
            if (files.length === 0)
                return `(no ${action.scope} files yet)`;
...
//# sourceMappingURL=manage_self.js.map