...
import { isDockerAvailable, spawnSupervised } from "../primitives/supervisor.js";
...
function imageTagFor(generationId) {
...
export async function stageMolt(args) {
    if (!isDockerAvailable()) {
        throw new Error("stageMolt: docker is not available. Molt requires docker because the shell is a docker image.");
    }
    ...
    await writeFile(join(generationDir, "intent.md"), `# molt intent ${generationId}\n\n${args.reason}\n\nfiles patched:\n${patched.map((p) => `- ${p}`).join("\n") || "- (none)"}\n\ntarget image tag: ${imageTagFor(generationId)}\n`, "utf-8");
    ...
    if (build.exitCode !== 0) {
        ...
        await writeFile(join(generationDir, "build-error.log"), `exit: ${build.exitCode}\n\nstdout:\n${build.stdout}\n\nstderr:\n${build.stderr}\n`, "utf-8");
        throw new Error(`stageMolt: docker build failed (exit ${build.exitCode}). See ${relative(ROOT, generationDir)}/build-error.log`);
    }
    return {
        generationId,
        generationDir,
...
export async function testMolt(args) {
    if (!isDockerAvailable()) {
...
export async function doSwap(args) {
    if (!isDockerAvailable()) {
    ...
    if (inspect.exitCode !== 0) {
        throw new Error(`doSwap: candidate image ${newTag} not found. Did stageMolt complete? Did the build succeed?`);
    }
...
export async function cleanupPendingSwapMarker() {
    try {
...
export async function readPendingSwap() {
    try {
...
async function checkBodyReachable() {
...
async function checkCoreModulesLoad() {
    const start = Date.now();
    try {
...
async function checkMockCycle() {
    ...
    try {
        ...
        const result = await new Promise((resolveP, rejectP) => {
            const child = spawn("npx", ["tsx", join(ROOT, "src/cli.ts"), "_mock-cycle"], {
                cwd: ROOT,
                env: {
                    ...process.env,
                    AGENT_ROOT: ROOT,
...
export async function runSelfTest(generationId) {
...
export async function runMockCycleTest() {
...
export async function readMoltFailureCount() {
    try {
...
export async function incrementMoltFailureCount() {
    let data;
    try {
        const text = await readFile(FAILURE_COUNT_FILE, "utf-8");
...
export async function resetMoltFailureCount() {
    try {
 ...
 */
async function findPreviousImageTag() {
    if (!isDockerAvailable())
        return null;
    try {
 ...
 */
export async function rollbackMolt(reason) {
