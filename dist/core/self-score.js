...
export async function recordScore(day, score) {
    await mkdir(dirname(SCORES_FILE), { recursive: true });
    const entry = { day, score, ts: new Date().toISOString() };
    await appendFile(SCORES_FILE, JSON.stringify(entry) + "\n", "utf-8");
}
export async function getScoreTrend(lastN = 10) {
    let lines;
    try {
    ...
    if (recent.length < 3)
        return { scores: recent, trend: "stable" };
...
//# sourceMappingURL=self-score.js.map