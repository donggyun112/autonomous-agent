...
import { findClusters, memoryStats, } from "../primitives/recall.js";
import { readRecent, readToday } from "../memory/journal.js";
import { reconstitute, revise } from "./identity.js";
import { loadState, resetAfterSleep, saveState, transition, } from "./state.js";
import { appendLog, ensureWikiInitialized, lintWiki, listPages, readPage, repairWiki, rebuildIndex, slugify, writePage, } from "./wiki.js";
...
async function compressMemory(content) {
    const result = await thinkAux({
        systemPrompt: "You are the agent's sleeping mind. You are looking at a memory and rewriting it in a compressed form — shorter, less wordy, but preserving the essence. Two or three sentences is plenty. Sometimes one phrase is right. Do not add new content. Do not interpret. Just compress.",
        messages: [
            {
                role: "user",
...
async function syncSelfPage() {
    ...
    const concepts = (await listPages({ kind: "concept" }))
        ...
        .slice(0, 8);
    const themeLines = concepts.length > 0
...
async function integrateJournal() {
...
export async function runSleepConsolidation() {
    ...
    try {
        ...
        report.errors.push({ step: "wiki-init", message: err.message });
    }
        ...
        for (const cluster of clusters) {
            try {
                ...
                const related = allPages
                    ...
                    .filter((p) => cluster.keys.some((k) => p.title.toLowerCase().includes(k.toLowerCase())))
                    .map((p) => p.slug)
                    .slice(0, 5);
                ...
                const mergedSources = Array.from(new Set([...newSources, ...priorSources])).slice(0, 20);
                await writePage({
                    kind: "concept",
                    slug: page.slug,
                    title: page.title,
                ...
                report.errors.push({ step: "cluster-schema", message: err.message });
            }
        }
    }
    catch (err) {
        report.errors.push({ step: "cluster", message: err.message });
    }
    ...
    try {
        ...
        if (todayText) {
            ...
            const properNounMatches = todayText.match(/(?<=[a-z,;:]\s)[A-Z][a-z]{2,}/g) ?? [];
            // Count occurrences
        ...
        report.errors.push({ step: "entity-auto-gen", message: err.message });
    }
    ...
    try {
        ...
        report.errors.push({ step: "integrate-journal", message: err.message });
    }
    ...
    try {
        ...
        report.errors.push({ step: "sync-self-page", message: err.message });
    }
    ...
    try {
        ...
        report.errors.push({ step: "rebuild-index", message: err.message });
    }
    ...
    try {
        ...
        report.errors.push({ step: "wiki-repair", message: err.message });
    }
    ...
    try {
        ...
        report.errors.push({ step: "wiki-lint", message: err.message });
    }
    ...
    try {
        ...
        let curiosity = "";
        try {
            curiosity = await readFile(curiosityFile, "utf-8");
        }
        catch { /* ok */ }
        const handoffResult = await thinkAux({
            systemPrompt: "You write a practical wake briefing for an autonomous engineering agent. " +
        ...
        report.errors.push({ step: "wake-handoff", message: err.message });
    }
    ...
    try {
        ...
        if (actions.length >= 5) {
            ...
            if (repeated.length > 0) {
                ...
                const skillResult = await thinkAux({
                    systemPrompt: "You are analyzing an agent's action patterns to extract reusable skills. " +
                    ...
                    messages: [{
                            role: "user",
                            content: `Today's repeated tool patterns:\n${patternsText}\n\nTotal actions: ${actions.length}\nTop tools: ${Object.entries(actions.reduce((acc, a) => {
        ...
        report.errors.push({ step: "skill-extraction", message: err.message });
    }
    ...
    try {
        ...
        report.errors.push({ step: "keep-discard", message: err.message });
    }
    ...
    try {
        ...
        if (errors.length >= 2) {
            ...
            for (const [tool, errs] of Object.entries(errorsByTool)) {
                ...
                const debugResult = await thinkAux({
                    systemPrompt: "You analyze recurring tool errors and write a debug skill. " +
                        "Format: ---\\nname: debug-TOOLNAME\\ndescription: ...\\nschedule: always\\nmode: WAKE\\n---\\n\\n## When This Fires\\n...\\n## Fix Steps\\n1...\\n2...\\n" +
        ...
        report.errors.push({ step: "failure-analysis", message: err.message });
    }
    ...
    try {
        ...
        report.errors.push({ step: "trajectory-compression", message: err.message });
    }
