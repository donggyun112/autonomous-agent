...
export function calculateSleepPressure(state, now = Date.now()) {
    ...
    if (combined < 0.25)
        level = "fresh";
    else if (combined < SOFT_THRESHOLD)
        level = "alert";
    else if (combined < STRONG_THRESHOLD)
        level = "tiring";
    else if (combined < FORCE_THRESHOLD)
        level = "tired";
    else
        level = "must-sleep";
...
export function tickAwake(state, now = Date.now()) {
    if (state.mode === "SLEEP") {
...
export function resetAfterSleep(state, now = Date.now()) {
    return {
        ...state,
        awakeMs: 0,
        awakeSince: now,
...
export async function transition(state, to, reason, options) {
