...
import { MEMORY_FILE, WIKI_CONCEPTS_DIR, WIKI_DIR, WIKI_ENTITIES_DIR, WIKI_INDEX, WIKI_LOG, WIKI_SELF, } from "../primitives/paths.js";
...
function buildWikiLookup(pages) {
...
function resolveWikiReference(raw, lookup) {
...
function shouldAutoLinkTitle(title) {
...
function bodyMentionsTitle(body, title) {
    if (!shouldAutoLinkTitle(title))
        return false;
...
function extractSourceTerms(page) {
...
async function loadStoredMemories() {
    try {
        const raw = JSON.parse(await readFile(MEMORY_FILE, "utf-8"));
...
function inferSourcesFromMemories(page, memories) {
    const terms = extractSourceTerms(page);
    if (terms.length === 0 || memories.length === 0)
        return [];
    ...
    const scored = memories
        ...
        let score = 0;
        if (titleLower.length >= 6 && contentLower.includes(titleLower))
            score += 4;
...
function pathForPage(kind, slug) {
    if (kind === "self")
        return WIKI_SELF;
...
function parseFrontmatter(text) {
    ...
    return {
        fm: {
            ...
            related: Array.isArray(obj.related) ? obj.related : undefined,
...
// ── Read/write ───────────────────────────────────────────────────────────
export async function readPage(kind, slug) {
...
async function listDir(dir, kind) {
    ...
    for (const name of entries) {
        ...
        if (kind === "meta") {
            ...
            if (base === "index" || base === "log" || base === "self")
                continue;
        }
...
export async function rebuildIndex() {
    ...
    if (grouped.concept.length > 0) {
        ...
        for (const p of grouped.concept) {
            lines.push(`- [${p.title}](concepts/${p.slug}.md) — updated ${p.updated_at}`);
        }
        lines.push("");
    }
    if (grouped.entity.length > 0) {
        ...
        for (const p of grouped.entity) {
            lines.push(`- [${p.title}](entities/${p.slug}.md) — updated ${p.updated_at}`);
        }
        lines.push("");
    }
    if (pages.length === 0) {
    ...
    for (let i = 0; i < pages.length; i++) {
        ...
        if (fm?.related) {
            for (const rel of fm.related) {
                ...
                if (!resolved)
                    continue;
                if (!relatedIndex.has(resolved.slug))
                    relatedIndex.set(resolved.slug, []);
        ...
        for (const indices of relatedIndex.values()) {
            for (let a = 0; a < indices.length; a++) {
                for (let b = a + 1; b < indices.length; b++) {
                    ...
                    if (pa.kind !== "concept" || pb.kind !== "concept")
                        continue;
                    if (!pa.body || !pb.body)
                        continue;
                    ...
                    for (const noun of sharedNouns.slice(0, 5)) {
                        ...
                        if (nounIdxA === -1 || nounIdxB === -1)
                            continue;
                        const windowA = pa.body.slice(Math.max(0, nounIdxA - 60), Math.min(pa.body.length, nounIdxA + noun.length + 60));
                        const windowB = pb.body.slice(Math.max(0, nounIdxB - 60), Math.min(pb.body.length, nounIdxB + noun.length + 60));
...
export async function repairWiki(options) {
    const pages = await listPages();
    const lookup = buildWikiLookup(pages);
    const memories = options?.backfillSources === false ? [] : await loadStoredMemories();
    const report = {
        totalPages: pages.length,
        pagesTouched: 0,
        relatedNormalized: 0,
    ...
    for (const summary of pages) {
        ...
        if (!relatedChanged && !sourcesChanged)
            continue;
        await writePage({
        ...
        if (relatedChanged)
            report.relatedNormalized += 1;
        if (sourcesChanged && (nextSources?.length ?? 0) > 0)
            report.sourcesBackfilled += 1;
    }
    return report;
}
// ── Log (chronological) ──────────────────────────────────────────────────
