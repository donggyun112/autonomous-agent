...
class MultiKeySource {
    ...
    constructor() {
        ...
        if (this.keys.length === 0) {
            throw new Error("No API keys available. Set ANTHROPIC_API_KEY (and optionally _2, _3) in .env.");
        }
    }
    describe() {
...
let _source = null;
export async function getAuthSource() {
    if (_source)
        return _source;
    ...
    if (mode === "auto") {
        ...
        throw new Error("No auth available. Either set ANTHROPIC_API_KEY or run 'pnpm run login' to authenticate via OAuth.");
    }
...
export function resetAuthSource() {
    _source = null;
}
