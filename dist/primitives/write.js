...
function checkWritable(target) {
    ...
    if (abs.startsWith(GENERATIONS))
        return;
    // src/extensions/ — light molt, allowed
    if (abs.startsWith(resolve(SRC, "extensions")))
        return;
    ...
    if (abs.startsWith(SRC)) {
        ...
        if (PROTECTED_SUBDIRS.includes(sub)) {
            throw new Error(`write: path is in protected shell core (${rel}). Use the molt ritual via generations/ instead.`);
        }
...
export async function writePath(path, content) {
    const abs = resolve(ROOT, path);
    checkWritable(abs);
    await mkdir(dirname(abs), { recursive: true });
    await writeFile(abs, content, "utf-8");
}
export async function appendPath(path, content) {
    const abs = resolve(ROOT, path);
    checkWritable(abs);
    await mkdir(dirname(abs), { recursive: true });
