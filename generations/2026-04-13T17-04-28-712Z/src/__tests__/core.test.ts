...
beforeAll(async () => {
  ...
  process.env.TIME_SCALE = "1"; // Tests expect real-time scale
...
describe("sleep pressure", () => {
  ...
  it("tickAwake accumulates time", async () => {
    const module = await import("../core/state.js");
    const { tickAwake, TIME_SCALE } = module as any;
    const base = {
    ...
    // wallDelta = 5000 - 1000 = 4000
    // agentDelta = 4000 * TIME_SCALE
    expect(ticked.awakeMs).toBe(4000 * TIME_SCALE); // accounts for TIME_SCALE
