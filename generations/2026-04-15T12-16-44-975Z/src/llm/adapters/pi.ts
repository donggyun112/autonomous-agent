...
function debugPiAdapter(label: string, data: unknown): void {
  try {
    console.log(`[PiAdapter:${label}]`, JSON.stringify(data));
  } catch {
    console.log(`[PiAdapter:${label}]`, String(data));
  }
}
...
export class PiAdapter implements LlmAdapter {
  ...
  async thinkOnce(args: ThinkOnceArgs): Promise<ThinkResult> {
    ...
    const stream = await streamSimple(model, context as any, {
      apiKey,
      promptCache: this.cacheRetention ?? "none",
    } as any);

    return await collectStream(stream as AsyncIterable<Record<string, unknown>>, args.onEvent);
  }

  async rotateCredential(): Promise<boolean> {
    if (!this.rotateCredentialFn) return false;
    return await this.rotateCredentialFn();
  }
}
