...
export type DriftReport = {
  score: number;          // 0..1, higher = further apart
  level: "still" | "growing" | "shifting" | "drifting" | "estranged";
  comparedAgainst: string; // snapshot filename
  comparedAgainstAge: string; // human-readable
};

function classify(score: number): DriftReport["level"] {
  if (score < 0.05) return "still";
  if (score < DRIFT_MEDIUM) return "growing";
  if (score < DRIFT_HIGH) return "shifting";
...
export async function measureDrift(
  against: "earliest" | "previous" | "midway" = "previous",
...
export async function birth(seedName: string): Promise<void> {
  const text = [
    "---",
