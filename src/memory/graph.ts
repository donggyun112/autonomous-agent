...
export class MemoryGraph {
  ...
  }): Promise<{ id: string; depth: number; previousContent: string }> {
    return this._lock.runExclusive(async () => {
      ...
      mem.depth = Math.min(
        DEPTH_MAX,
        mem.depth + (args.depthIncrement ?? 0.2),
      );
  ...
  listRecent(limit = 20): { id: string; content: string; depth: number; created_at: number; access_count: number }[] {
    return Object.entries(this.memories)
  ...
  listShallow(maxDepth = 0.3, limit = 20): { id: string; content: string; depth: number; created_at: number }[] {
    return Object.entries(this.memories)
      .filter(([mid, mem]) => {
        if (this._isExpired(mem) || mid in this._supersededBy) return false;
  ...
  async pruneWeak(options: { minAgeSec?: number; maxToPrune?: number } = {}): Promise<string[]> {
    ...
    return this._lock.runExclusive(async () => {
      ...
      const candidates = Object.entries(this.memories).filter(([mid, mem]) => {
        ...
        if (mem.depth >= 0.05) return false;
        if (now - mem.created_at < minAgeSec) return false;
        return true;
      });
      ...
      this._pruneOrphanKeys();
      if (toPrune.length > 0) await this.save();
      return toPrune;
    });
  }
  ...
  findClusters(options: { minSharedKeys?: number; minClusterSize?: number; maxClusters?: number } = {}): {
    keys: string[];
    memoryIds: string[];
    contents: string[];
  }[] {
    ...
    const keyIds = Object.keys(this.keys);
    const clusters: { keys: string[]; memoryIds: string[]; contents: string[] }[] = [];
    ...
    for (const c of clusters) {
      ...
      if (!overlapsExisting) kept.push(c);
      if (kept.length >= maxClusters) break;
    }
  ...
  pickRandomDistantPair(): { a: { id: string; content: string }; b: { id: string; content: string } } | null {
  ...
  async link(memA: string, memB: string, viaKey: string): Promise<void> {
    return this._lock.runExclusive(async () => {
      const a = this.memories[memA];
      const b = this.memories[memB];
      if (!a || !b) return;
      if (!a.links.includes(memB)) a.links.push(memB);
      if (!b.links.includes(memA)) b.links.push(memA);
      const kid = await this.findOrCreateKey(viaKey, "concept");
      if (!this._hasLink(kid, memA)) this._link(kid, memA);
      if (!this._hasLink(kid, memB)) this._link(kid, memB);
      await this.save();
    });
  }
